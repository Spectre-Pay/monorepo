...
var abytes = (value2, length, title = "") => {
...
class SHA2_32B extends HashMD {
  ...
  get() {
    const { A, B, C: C2, D, E: E2, F, G: G2, H } = this;
    return [A, B, C2, D, E2, F, G2, H];
  }
  set(A, B, C2, D, E2, F, G2, H) {
    this.A = A | 0;
    this.B = B | 0;
    this.C = C2 | 0;
    this.D = D | 0;
  ...
  process(view, offset) {
    ...
    let { A, B, C: C2, D, E: E2, F, G: G2, H } = this;
    for (let i2 = 0;i2 < 64; i2++) {
      const sigma1 = rotr(E2, 6) ^ rotr(E2, 11) ^ rotr(E2, 25);
      const T1 = H + sigma1 + Chi(E2, F, G2) + SHA256_K[i2] + SHA256_W[i2] | 0;
...
function createKeygen2(randomSecretKey2, getPublicKey2) {
...
function weierstrass(params, extraOpts = {}) {
  ...
  const lengths2 = getWLengths(Fp, Fn);
  ...
  class Point2 {
    ...
    static fromBytes(bytes) {
      const P2 = Point2.fromAffine(decodePoint(abytes2(bytes, undefined, "point")));
      P2.assertValidity();
      return P2;
    }
    static fromHex(hex) {
...
function ecdh(Point2, ecdhOpts = {}) {
  ...
  const lengths2 = Object.assign(getWLengths(Point2.Fp, Fn), { seed: getMinHashLength(Fn.ORDER) });
  ...
  function randomSecretKey2(seed = randomBytes_(lengths2.seed)) {
    return mapHashToField(abytes2(seed, lengths2.seed, "seed"), Fn.ORDER);
  }
  ...
  const keygen2 = createKeygen2(randomSecretKey2, getPublicKey2);
  return Object.freeze({ getPublicKey: getPublicKey2, getSharedSecret: getSharedSecret2, keygen: keygen2, Point: Point2, utils: utils2, lengths: lengths2 });
}
function ecdsa(Point2, hash, ecdsaOpts = {}) {
  ...
  const { keygen: keygen2, getPublicKey: getPublicKey2, getSharedSecret: getSharedSecret2, utils: utils2, lengths: lengths2 } = ecdh(Point2, ecdsaOpts);
  ...
  class Signature2 {
    ...
    recoverPublicKey(messageHash) {
      ...
      const h = bits2int_modN2(abytes2(messageHash, undefined, "msgHash"));
  ...
  const bits2int_modN2 = ecdsaOpts.bits2int_modN || function bits2int_modN_def(bytes) {
    return Fn.create(bits2int2(bytes));
  };
  ...
  function prepSig(message, secretKey, opts) {
    ...
    function k2sig(kBytes) {
      ...
      return new Signature2(r, normS, hasLargeCofactor ? undefined : recovery);
    }
    return { seed, k2sig };
  }
...
var Fpk1 = Field(secp256k1_CURVE2.p, { sqrt: sqrtMod });
var Pointk1 = /* @__PURE__ */ weierstrass(secp256k1_CURVE2, {
  Fp: Fpk1,
  endo: secp256k1_ENDO
});
...
class _SHA1 extends HashMD {
  ...
  process(view, offset) {
    ...
    for (let i2 = 0;i2 < 80; i2++) {
      ...
      const T = rotl(A, 5) + F + E2 + K + SHA1_W[i2] | 0;
      E2 = D;
      D = C2;
      C2 = rotl(B, 30);
      B = A;
...
class _RIPEMD160 extends HashMD {
  ...
  process(view, offset) {
    ...
    for (let group = 0;group < 5; group++) {
      ...
      for (let i2 = 0;i2 < 16; i2++) {
        const tr = rotl(ar + ripemd_f(rGroup, br, cr2, dr) + BUF_160[rr[i2]] + hbr, sr[i2]) + er | 0;
...
var generateEphemeralPrivateKey = (viewingKeyNode, nonce, chainId) => {
  ...
  const MAX_NONCE = BigInt(268435455);
